...
void Robot::load_config()
{
	...
	*/
    ...
    this->default_acceleration= THEKERNEL->config->value(acceleration_checksum)->by_default(50.0F )->as_number(); // Acceleration is in mm/s^2
    ...
    soft_endstop_max[X_AXIS]= NAN;
    soft_endstop_max[Y_AXIS]= NAN;
    soft_endstop_max[Z_AXIS]= NAN;
...
bool Robot::append_milestone(const float target[], float rate_mm_s, unsigned int line)
{
    ...
    if(soft_endstop_enabled && !THEKERNEL->is_zprobing()) {
    	float soft_endstop_min_backup = soft_endstop_min[Y_AXIS];
    	if(((MAKERA_Z1 == THEKERNEL->factory_set->MachineModel)||(MAKERA_Z1Pro == THEKERNEL->factory_set->MachineModel)) && THEKERNEL->axis_is_on[A_AXIS])
    	{
    		soft_endstop_min[Y_AXIS] = -160;
    	}
        ...
        soft_endstop_min[Y_AXIS] = soft_endstop_min_backup;
    }
...
//debug
//THEKERNEL->streams->printf("Add to block line: %d\n", line);
        return true;
    }
...
bool Robot::is_homed_all_axes()
{
    ...
    for (int i = X_AXIS; i <= Z_AXIS; ++i) {
        if (!this->is_homed(i)){
            THEKERNEL->streams->printf("ERROR:Machine has not been homed,Please home first!\nUse M888 To disable homed check temporarily\n");
